...
public class ServPerfil {
    ...
    public PerfilDTO updateDTO(PerfilDTO perfilDTO) throws PerfilException, ErrorSQL {
        ...
        //
        if (!perfilRepository.findByDniOrEmail(perfil.getDni(), perfil.getEmail()).isPresent()) {
            throw new PerfilException("no existe :" + perfil.getDni() + " " + perfil.getEmail());
        }
        try {
            perfil.setId(perfilRepository.findByDni(perfil.getDni()).get().getId());
            perfil = perfilRepository.save(perfil);
            // se hace esto para no tener ocupado a la base de datos
            if (!imgRepo.findByPerfilAndType(perfil, ImageType.BackGround).isEmpty()) {
                temp = imgRepo.findByPerfilAndType(perfil, ImageType.BackGround).get();
    ...
    public PerfilDTO borrarDTO(PerfilDTO pDto) throws PerfilException {
        if (perfilRepository.findByDniOrEmail(pDto.getDni(), pDto.getEmail()).isPresent()) {
            perfilRepository.delete(perfilRepository.findByDni(pDto.getDni()).get());
            return pDto;
        }
        throw new PerfilException("no existe" + pDto.getDni());
    }
